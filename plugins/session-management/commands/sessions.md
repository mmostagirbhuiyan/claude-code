...
```bash
if [ -n "$ARGUMENTS" ]; then
  target_dir=$(eval echo "$ARGUMENTS")
else
  target_dir="$(pwd)"
fi
encoded_dir=$(echo "$target_dir" | sed 's|/|-|g')
...
if [ -d "$project_dir" ]; then
  ...
  for f in $(ls -t "$project_dir"/*.jsonl 2>/dev/null); do
    ...
    preview=$(grep -m1 '"type":"user"' "$f" 2>/dev/null | python3 -c "import sys,json; d=json.loads(sys.stdin.readline()); c=d.get('message',{}).get('content',''); print(c[:80] if isinstance(c,str) else str(c)[:80])" 2>/dev/null || echo "(no preview)")
    echo ""
    echo "  ID: $session_id"
  ...
  ls -dt "$HOME/.claude/projects"/*/ 2>/dev/null | head -10 | while read dir; do
    dirname=$(basename "$dir")
    decoded=$(echo "$dirname" | sed 's|-|/|g')
